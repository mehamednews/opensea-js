        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    proxy = _a.sent();
                    if (!proxy) {
                        console.warn("No proxy found for seller ".concat(seller));
                        return [2 /*return*/, false];
                    }
                    contract = client.web3.eth.contract([Proxy_1.proxyABI]).at(proxy);
                    return [2 /*return*/, promisify(function (c) { return contract.proxy.call(order.target, order.howToCall, calldata, { from: seller }, c); })];
            }
        });
    });
}
