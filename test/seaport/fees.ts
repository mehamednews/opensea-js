...
suite('seaport: fees', () => {
	...
	test('Errors for computing fees correctly', async () => {
		try {
			...
			if (!error.message.includes('bounty exceeds the maximum') || !error.message.includes('OpenSea will add')) {
	...
	test('_getBuyFeeParameters works for assets', async () => {
		...
		const { totalBuyerFeeBasisPoints, totalSellerFeeBasisPoints } = await client.computeFees({ asset, extraBountyBasisPoints, side: OrderSide.Buy });

		const { makerRelayerFee, takerRelayerFee, makerProtocolFee, takerProtocolFee, makerReferrerFee, feeRecipient, feeMethod } = client._getBuyFeeParameters(
