...
export class OpenSeaPort {
  ...
  constructor(provider: Web3.Provider, apiConfig: OpenSeaAPIConfig = {}, logger?: (arg: string) => void) {
    ...
    this._wrappedNFTLiquidationProxyAddress =
      this._networkName == Network.Main ? WRAPPED_NFT_LIQUIDATION_PROXY_ADDRESS_MAINNET : WRAPPED_NFT_LIQUIDATION_PROXY_ADDRESS_RINKEBY
  ...
  public addListener(event: EventType, listener: (data: EventData) => void, once = false): EventSubscription {
    const subscription = once ? this._emitter.once(event, listener) : this._emitter.addListener(event, listener)
    return subscription
  }
  ...
  /**
	 ...
	 */
  public async wrapAssets({ assets, accountAddress }: { assets: Asset[]; accountAddress: string }) {
    ...
    const txHash = await sendRawTransaction(
      this.web3,
      {
        ...
        value: 0,
        data: encodeCall(getMethod(WrappedNFTLiquidationProxy, 'wrapNFTs'), [tokenIds, tokenAddresses, isMixedBatchOfAssets]),
      }, error => {
  ...
  /**
	 ...
	 * @param accountAddress Address of the user's wallet
	 */
  public async unwrapAssets({ assets, destinationAddresses, accountAddress }: { assets: Asset[]; destinationAddresses: string[]; accountAddress: string }) {
    ...
    const txHash = await sendRawTransaction(
      this.web3,
      {
        ...
        to: this._wrappedNFTLiquidationProxyAddress,
        value: 0,
        data: encodeCall(getMethod(WrappedNFTLiquidationProxy, 'unwrapNFTs'), [tokenIds, tokenAddresses, destinationAddresses, isMixedBatchOfAssets]),
      }, error => {
        this._dispatch(EventType.TransactionDenied, { error, accountAddress })
      }
    )
  ...
  public async liquidateAssets({
    ...
    const uniswapSlippage =
      uniswapSlippageAllowedInBasisPoints === 0 ? DEFAULT_WRAPPED_NFT_LIQUIDATION_UNISWAP_SLIPPAGE_IN_BASIS_POINTS : uniswapSlippageAllowedInBasisPoints
    ...
    const txHash = await sendRawTransaction(
      this.web3,
      {
        ...
        to: this._wrappedNFTLiquidationProxyAddress,
        value: 0,
        data: encodeCall(getMethod(WrappedNFTLiquidationProxy, 'liquidateNFTs'), [tokenIds, tokenAddresses, isMixedBatchOfAssets, uniswapSlippage]),
      }, error => {
        this._dispatch(EventType.TransactionDenied, { error, accountAddress })
      }
    )
  ...
  public async purchaseAssets({
    ...
    const txHash = await sendRawTransaction(
      this.web3,
      {
        ...
        to: this._wrappedNFTLiquidationProxyAddress,
        value: amount,
        data: encodeCall(getMethod(WrappedNFTLiquidationProxy, 'purchaseNFTs'), [numTokensToBuy, contractAddress]),
      }, error => {
        this._dispatch(EventType.TransactionDenied, { error, accountAddress })
      }
    )
  ...
  /**
	 ...
	 * @param contractAddress Address of the corresponding NFT core contract for these NFTs.
	 */
  public async getQuoteFromUniswap({ numTokens, isBuying, contractAddress }: { numTokens: number; isBuying: boolean; contractAddress: string }) {
  ...
  public async isAssetTransferrable(
    {
      ...
      quantity,
      useProxy = false,
    }: { asset: Asset; fromAddress: string; toAddress: string; quantity?: number | BigNumber; useProxy?: boolean },
    retries = 1
  ): Promise<boolean> {
    const schema = this._getSchema(asset.schemaName)
    const quantityBN = quantity ? WyvernProtocol.toBaseUnitAmount(makeBigNumber(quantity), asset.decimals || 0) : makeBigNumber(1)
  ...
  public async transfer({
    ...
    const isOldNFT = isCryptoKitties || (!!asset.version && [TokenStandardVersion.ERC721v1, TokenStandardVersion.ERC721v2].includes(asset.version))
  ...
  /**
	 ...
	 */
  public async getFungibleTokens({ symbol, address, name }: { symbol?: string; address?: string; name?: string } = {}): Promise<OpenSeaFungibleToken[]> {
    ...
    const offlineTokens: OpenSeaFungibleToken[] = [tokenSettings.canonicalWrappedEther, ...tokenSettings.otherTokens].filter(t => {
  ...
  public async getTokenBalance(
    { accountAddress, tokenAddress, schemaName = WyvernSchemaName.ERC20 }: { accountAddress: string; tokenAddress: string; schemaName?: WyvernSchemaName },
    retries = 1
  ) {
    const asset: Asset = {
      tokenId: null,
  ...
  public async computeFees({
    ...
    if (side == OrderSide.Sell && asset) {
      ...
      transferFee = asset.transferFee ? makeBigNumber(asset.transferFee) : transferFee
  ...
  public async _estimateGasForMatch(
    { buy, sell, accountAddress, metadata = NULL_BLOCK_HASH }: { buy: Order; sell: Order; accountAddress: string; metadata?: string },
    retries = 1
    ...
    try {
      return await this._getClientsForRead(retries).wyvernProtocol.wyvernExchange.atomicMatch_.estimateGasAsync(
        ...
        [buy.v || 0, sell.v || 0],
        [buy.r || NULL_BLOCK_HASH, buy.s || NULL_BLOCK_HASH, sell.r || NULL_BLOCK_HASH, sell.s || NULL_BLOCK_HASH, metadata],
  ...
  public async _getApprovedTokenCount({
    ...
    const approved = await rawCall(this.web3, {
      from: accountAddress,
      to: tokenAddress,
      data: encodeCall(getMethod(ERC20, 'allowance'), [accountAddress, addressToApprove]),
    })
  ...
  public async _makeBuyOrder({
    ...
    const { totalBuyerFeeBasisPoints, totalSellerFeeBasisPoints } = await this.computeFees({
      asset: openSeaAsset,
      extraBountyBasisPoints,
      side: OrderSide.Buy,
    })

    const { makerRelayerFee, takerRelayerFee, makerProtocolFee, takerProtocolFee, makerReferrerFee, feeRecipient, feeMethod } = this._getBuyFeeParameters(
  ...
  public async _makeSellOrder({
    ...
    const { basePrice, extra, paymentToken, reservePrice } = await this._getPriceParameters(
      OrderSide.Sell,
      paymentTokenAddress,
      expirationTime,
      startAmount,
    ...
    const { makerRelayerFee, takerRelayerFee, makerProtocolFee, takerProtocolFee, makerReferrerFee, feeRecipient, feeMethod } = this._getSellFeeParameters(
      totalBuyerFeeBasisPoints,
      totalSellerFeeBasisPoints,
      waitForHighestBid,
      sellerBountyBasisPoints
  ...
  public async _getStaticCallTargetAndExtraData({ asset, useTxnOriginStaticCall }: { asset: OpenSeaAsset; useTxnOriginStaticCall: boolean }): Promise<{
    staticTarget: string;
    staticExtradata: string;
  }> {
    const isCheezeWizards = [CHEEZE_WIZARDS_GUILD_ADDRESS.toLowerCase(), CHEEZE_WIZARDS_GUILD_RINKEBY_ADDRESS.toLowerCase()].includes(
    ...
    if (isCheezeWizards) {
      ...
      return {
        ...
        staticExtradata: encodeCall(getMethod(StaticCheckCheezeWizards, 'succeedIfCurrentWizardFingerprintMatchesProvidedWizardFingerprint'), [
          asset.tokenId,
          wizardFingerprint,
      ...
      return {
        ...
        staticExtradata: encodeCall(getMethod(StaticCheckDecentralandEstates, 'succeedIfCurrentEstateFingerprintMatchesProvidedEstateFingerprint'), [
          asset.tokenId,
          estateFingerprint,
          useTxnOriginStaticCall,
        ]),
  ...
  public async _makeBundleBuyOrder({
    ...
    const { totalBuyerFeeBasisPoints, totalSellerFeeBasisPoints } = await this.computeFees({ asset, extraBountyBasisPoints, side: OrderSide.Buy })
  ...
  public async _makeBundleSellOrder({
    ...
    const { makerRelayerFee, takerRelayerFee, makerProtocolFee, takerProtocolFee, makerReferrerFee, feeRecipient } = this._getSellFeeParameters(
      totalBuyerFeeBasisPoints,
      totalSellerFeeBasisPoints,
      waitForHighestBid,
      sellerBountyBasisPoints
  ...
  public async _validateMatch(
    ...
    }: { buy: Order; sell: Order; accountAddress: string; shouldValidateBuy?: boolean; shouldValidateSell?: boolean },
    retries = 1
  ): Promise<boolean> {
    try {
  ...
  public async _createEmailWhitelistEntry({ order, buyerEmail }: { order: UnhashedOrder; buyerEmail: string }) {
  ...
  // Throws
  public async _sellOrderValidationAndApprovals({ order, accountAddress }: { order: UnhashedOrder; accountAddress: string }) {
  ...
  public _getBuyFeeParameters(totalBuyerFeeBasisPoints: number, totalSellerFeeBasisPoints: number, sellOrder?: UnhashedOrder) {
    ...
    if (sellOrder) {
      ...
      makerRelayerFee = sellOrder.waitingForBestCounterOrder ? makeBigNumber(sellOrder.makerRelayerFee) : makeBigNumber(sellOrder.takerRelayerFee)
      takerRelayerFee = sellOrder.waitingForBestCounterOrder ? makeBigNumber(sellOrder.takerRelayerFee) : makeBigNumber(sellOrder.makerRelayerFee)
    } else {
  ...
  public _getSellFeeParameters(totalBuyerFeeBasisPoints: number, totalSellerFeeBasisPoints: number, waitForHighestBid: boolean, sellerBountyBasisPoints = 0) {
    ...
    const makerRelayerFee = waitForHighestBid ? makeBigNumber(totalBuyerFeeBasisPoints) : makeBigNumber(totalSellerFeeBasisPoints)
    const takerRelayerFee = waitForHighestBid ? makeBigNumber(totalSellerFeeBasisPoints) : makeBigNumber(totalBuyerFeeBasisPoints)
  ...
  private async _atomicMatch({
    ...
    try {
      ...
      const gasEstimate = await this._wyvernProtocolReadOnly.wyvernExchange.atomicMatch_.estimateGasAsync(
        args[0],
        args[1],
        args[2],
        args[3],
  ...
  private async _authorizeOrder(order: UnsignedOrder): Promise<ECSignature | null> {
  ...
  /**
	 ...
	 */
  private _getClientsForRead(retries = 1): { web3: Web3; wyvernProtocol: WyvernProtocol } {
    if (retries > 0) {
